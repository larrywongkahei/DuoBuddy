...
export const APIService =
{
  ...
  async signin(email: string, password: string) {
    ...
    sessionStorage.setItem("userId", data.id)
    return window.location.href = `https://larrywongkahei.github.io/DuoBuddy?userId=${data.id}`
  },
  ...
  async fetchUserOrCreateUser(name: string, bio: string | null, location: string | null, avatar_url: string, email: string | null, password: string | null) {
    ...
    if (fetchdata.status === 200) {
      const data = await fetchdata.json()
      sessionStorage.setItem("userId", data.id)
      window.location.href = `https://larrywongkahei.github.io/DuoBuddy?userId=${data.id}`
    }
    else {
  ...
  getUsersByProjectIds(projectId:string){
