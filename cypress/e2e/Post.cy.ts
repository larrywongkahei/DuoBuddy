...
describe('Test about posts that require loggin', () => {
    beforeEach(() => {
        ...
        const comments = cy.get('.eachComment').its('length')
        cy.log(comments)
        cy.get(".CommentBox").type("Test Comment")
        cy.get(`[type = 'submit']`).click()
        cy.get('.CommentSessionContainer > p').click().wait(5000)
        cy.get('.eachComment').its('length').then((length) => {
            cy.log(length)
            comments.should('equal', length - 1)
        })
    })
